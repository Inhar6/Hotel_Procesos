...
public class HabitacionServiceDisponiblesPerformanceTest {
    ...
    private static final JUnitPerfReportingConfig PERF_CONFIG = JUnitPerfReportingConfig.builder()
            .reportGenerator(new HtmlReportGenerator(System.getProperty("user.dir") + "/target/reports/habitacionDisponiblesPerf-report.html"))
            .build();

    @Test
    @JUnitPerfTest(threads = 10, durationMs = 3000, warmUpMs = 500)
    @JUnitPerfTestRequirement(allowedErrorPercentage = 0.05f)
    public void testListarHabitacionesDisponiblesPerformance() {
        List<Habitacion> habitaciones = habitacionService.getHabitacionesDisponibles();
    }
}